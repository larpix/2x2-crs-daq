...
def get_io(c, io_channel, io_group):
    c.io=larpix.io.PACMAN_IO(relaxed=True)
    c.io.set_reg(0x18, 2**(io_channel-1), io_group=io_group)
    return c.io

def haha(c, io, chip_key, channel, reset_cycles):
    c[chip_key].config.csa_enable=[1]*64
    c.write_configuration(chip_key,'csa_enable')
    c[chip_key].config.enable_periodic_reset=1
    c.write_configuration(chip_key,'enable_periodic_reset')
    c[chip_key].config.periodic_reset_cycles=reset_cycles
    c.write_configuration(chip_key,'periodic_reset_cycles')
    c[chip_key].config.vref_dac=223
    c.write_configuration(chip_key,'vref_dac')
    c[chip_key].config.vcm_dac=68
    c.write_configuration(chip_key,'vcm_dac')    
    c.enable_analog_monitor(chip_key, channel)
...
#                             warm_wait=20)
    ...
    _io_group_pacman_tile_={2:[1,2,3,4]}
#    _io_group_pacman_tile_={1:[3],2:[3]}
