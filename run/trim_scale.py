...
def main(LRS=_default_LRS, \
    ...
    pixel_dac=[0,5,10,15,20,25,30]
    for trim in pixel_dac:
        
        for g_c in iog_ioc.keys():
            ok, diff=asic_base.enable_response_trigger_config_by_io_channel(c, io,\
                                                                            iog_ioc[g_c],
                                                                            vref_dac, vcm_dac, vdda, trim, periodic_reset_cycles, pedestal, disabled, target)
        ...
        while ctr<file_count:
            filename = utility_base.data(c, runtime, False, file_prefix+'_trim_dac_'+str(trim), LRS)
        ...
        for g_c in iog_ioc.keys():
            ...
            if not ok:
                for key in diff.keys():
                    print('MISCONFIGURED ASIC: ',key)
                    print('*****WARNING: Response-based triggering NOT turned \
off.*****\n')
