...
def main(LRS=_default_LRS, \
         ...
         i_rx=_default_i_rx, \
         burst_length=_default_burst_length, \
    ...
    for g_c in iog_ioc.keys():
        io.set_reg(0x18, 2**(g_c[1]-1), io_group=g_c[0])
        for chip_key in iog_ioc[g_c]:
            c[chip_key].config.adc_burst_length=burst_length
            c.write_configuration(chip_key,'adc_burst_length')
            c[chip_key].config.adc_hold_delay=adc_hold_delay
...
if __name__=='__main__':
    ...
    parser.add_argument('--burst_length', default=_default_burst_length, \
                        type=int, help='''ADC burst length [MCLK]''')
    parser.add_argument('--adc_hold_delay', default=_default_adc_hold_delay,\
                        type=int, help='''ADC hold delay [MCLK]''')
