...
def main(input_filename, output_filename, block_size):
    files = os.listdir(input_filename)
    converted_files = os.listdir(output_filename)
    for file in files:
        print('found', file)
        if 'packet_'+file in converted_files: continue
        if os.path.isdir(input_filename + '/' + file): continue
        print('converting\t', input_filename + '/' + file, '\t------->\t',  output_filename + '/packet_' + file)
        try:
            main_single(input_filename + '/' + file,  output_filename + '/packet_' + file, block_size)
        except:
            continue


def main_single(input_filename, output_filename, block_size):
